...
1 procesador
4 GB de memoria (4 bloques de 1 GB = 1024 MB cada uno)
Funcionalidades:

Solicitud y liberación de recursos por parte de los procesos.
Detección y prevención de conflictos (recursos no disponibles)
Política de prevención de interbloqueo: no permitir solicitudes que excedan los recursos disponibles
...
Round Robin: Con configuración cuántica
...
Normal (proceso completo su ejecución)
Error
Punto muerto
...
Mostrar registros de eventos (cambios de estado, asignación de recursos)
...
Rendimiento: Procesos completados por unidad de tiempo
...
2. Round Robin
Si selecciona Round Robin, deberá especificar la cantidad.
...
3 semáforos (mutex, vacío, lleno)
Búfer compartido de tamaño 5
Utilice "Ejecutar siguiente paso" para ver la interacción.
...
PCB: Bloque de Control de Procesos - información de cada proceso
Recursos: CPU y memoria
Mensaje: Estructura de mensajes entre procesos
Semáforo: Semáforo con cola de espera
SharedBuffer: Buffer circular para productor-consumidor
LogEntry: Entrada de registro con marca de tiempo
Sistema: Estado global del sistema
Funciones principales
init_system(): Inicializa el sistema
create_process(): Crea un nuevo proceso
execute_step(): Ejecuta un paso de simulación
select_next_process(): Selecciona el siguiente proceso según algoritmo
...
show_statistics(): Muestra estadisticas de rendimiento
...
Esto evita la espera circular y el punto muerto
Los procesos que no pueden obtener recursos inmediatamente pasan a estado ESPERANDO
Limitaciones
...
Notas Técnicas
Estados de Proceso
LISTO: Proceso listo para ejecutar, esperando CPU
RUNNING: Proceso en ejecución
ESPERANDO: Proceso esperando recursos o eventos
...
Sin envejecimiento (posible inanición)
